...
public interface PriceDao extends JpaRepository<PriceEntity, Long> {
    ...
    @Query("select new java.lang.Boolean(count(*) > 0) from EntrepriseEntity ent, PriceEntity p where ent.libelle = :libelle and p.createdAt <= :createdAt and ent.prixActuel = p.id")
